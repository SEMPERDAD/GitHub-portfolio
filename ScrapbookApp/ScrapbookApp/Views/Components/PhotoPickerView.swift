    ...
    var body: some View {
        NavigationStack {
            VStack(spacing: 24) {
                ...
                Text("Import Photos").font(.title2.weight(.bold))

                Text("Select photos from your library to add to this page.")
                ...
                PhotosPicker(selection: $selectedItems, maxSelectionCount: 20, matching: .images) {
                    Label("Choose Photos", systemImage: "photo.on.rectangle.angled")
                        ...
                        .background(.pink).foregroundStyle(.white)
        ...
        for item in items {
            if let data = try? await item.loadTransferable(type: Data.self),
               let image = UIImage(data: data) {
                await MainActor.run { viewModel.addPhoto(image, to: pageId, in: bookId) }
            }
        }
        isImporting = false
        selectedItems = []
