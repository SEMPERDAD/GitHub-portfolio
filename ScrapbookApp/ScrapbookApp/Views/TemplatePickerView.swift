...
private struct TemplateCard: View {
    ...
    var body: some View {
        ...
        .onLongPressGesture(minimumDuration: 0, maximumDistance: .infinity, pressing: { pressed = $0 }, perform: {})
    }
}

// MARK: - Template Preview

/// Renders a scaled-down live preview of the template layout.
/// Items are drawn using the same offset coordinate system as the real canvas
/// (origin at center), then scaled to fit the preview bounds and clipped.
private struct TemplatePreviewView: View {
    ...
    private func itemView(_ item: ScrapbookItem, scale: CGFloat) -> some View {
        switch item.type {
        case .photo:
            photoThumb(item, scale: scale)
        case .text:
            textThumb(item, scale: scale)
        case .sticker:
            stickerThumb(item, scale: scale)
        }
    }
