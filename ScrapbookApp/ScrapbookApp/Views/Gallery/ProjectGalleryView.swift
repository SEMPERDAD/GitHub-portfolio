...
struct ProjectGalleryView: View {
    ...
    var filteredBooks: [Scrapbook] {
        searchText.isEmpty
            ? viewModel.scrapbooks
            : viewModel.scrapbooks.filter { $0.title.localizedCaseInsensitiveContains(searchText) }
    }

    var body: some View {
        ZStack(alignment: .bottomTrailing) {
            if viewModel.scrapbooks.isEmpty && searchText.isEmpty {
                emptyState
            } else {
                scrollContent
            }
            ...
            Button { showingNewBook = true } label: {
                Image(systemName: "plus")
                    ...
                    .clipShape(Circle())
                    .shadow(color: Color(red: 1.0, green: 0.30, blue: 0.52).opacity(0.42),
                            radius: 14, x: 0, y: 6)
            }
            .padding(.trailing, 24)
            .padding(.bottom, 28)
        ...
        .alert("New Scrapbook", isPresented: $showingNewBook) {
            ...
            Button("Create") {
                if !newTitle.trimmingCharacters(in: .whitespaces).isEmpty {
                    _ = viewModel.createScrapbook(title: newTitle)
                    newTitle = ""
                }
            }
        ...
        ScrollView {
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 16) {
                ForEach(filteredBooks) { book in
                    NavigationLink(destination: ScrapbookEditorView(scrapbook: book)) {
                        ScrapbookCard(scrapbook: book)
                    }
                    .buttonStyle(.plain)
                    .contextMenu {
                        Button(role: .destructive) {
                            viewModel.deleteScrapbook(book)
                        } label: {
...
struct ScrapbookCard: View {
    ...
    private var coverGradient: LinearGradient {
        let palettes: [[Color]] = [
            [Color(red: 1.00, green: 0.78, blue: 0.82), Color(red: 0.97, green: 0.62, blue: 0.72)],
            [Color(red: 0.86, green: 0.79, blue: 0.97), Color(red: 0.74, green: 0.62, blue: 0.92)],
            [Color(red: 1.00, green: 0.89, blue: 0.72), Color(red: 1.00, green: 0.76, blue: 0.58)],
            [Color(red: 0.76, green: 0.92, blue: 0.89), Color(red: 0.60, green: 0.84, blue: 0.84)],
        ]
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ZStack {
                coverGradient.frame(height: 170)
...
#Preview {
    NavigationStack { ProjectGalleryView() }
